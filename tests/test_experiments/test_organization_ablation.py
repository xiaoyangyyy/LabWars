...
def test_organization_ablation_runs_core_lesions():
    result = run_organization_ablation(
        ...
        conditions=["full", "memory_lesion", "hierarchy_lesion", "status_lesion", "trust_lesion", "social_physics_only", "llm_scoring_off"],
        seeds=[0],
    ...
    assert set(result.summary) == {"full", "memory_lesion", "hierarchy_lesion", "status_lesion", "trust_lesion", "social_physics_only", "llm_scoring_off"}
    assert len(result.per_seed) == 7
